...
import { Key, Loader, AlertTriangle, ShieldCheck, Info, Brain } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <header className="text-center mb-10">
        <h1 className="text-4xl font-bold tracking-tight text-slate-900 flex items-center justify-center">
          <Key className="w-8 h-8 mr-3 text-rose-500" /> Password Exposure Analysis
        </h1>
        <p className="text-slate-500 mt-2 text-lg">Check if your password has been exposed in known data breaches (k-anonymity check).</p>
      </header>
        ...
        <GlassCard className="p-6">
          <form onSubmit={handleAnalyze} className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-grow w-full">
              ...
              <input type="password"
                className="w-full pl-12 pr-4 py-3 rounded-full border border-slate-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-rose-400 focus:border-transparent transition-all shadow-sm"
                placeholder="Enter password to check..."
                value={password} onChange={(e) => setPassword(e.target.value)} disabled={isAnalyzing} />
            </div>
            <Button type="submit" disabled={isAnalyzing || !password} className="w-full md:w-auto bg-slate-900 text-white rounded-full px-8 py-6 font-semibold hover:bg-slate-800 shadow-md transition-all hover:-translate-y-0.5">
              {isAnalyzing ? <><Loader className="w-5 h-5 mr-2 animate-spin" /> Analyzing...</> : <><Key className="w-5 h-5 mr-2" /> Check Password</>}
            </Button>
          </form>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
            <GlassCard className="p-8">
              ...
              <div className="space-y-5">
                <div>
                  <h3 className="text-base font-bold text-slate-800 flex items-center mb-2"><Brain className="w-5 h-5 mr-2 text-rose-500" /> AI Security Summary</h3>
            ...
            <CyberEduBox moduleType="password" result={result} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
